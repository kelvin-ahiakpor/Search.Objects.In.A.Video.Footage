...
def combine_files(input_dir, output_file):
    # List and sort chunk files using a regex pattern to extract the numeric part
    pattern = re.compile(r'inception_v3_weights\.weights\.(\d+)\.h5')
    chunk_files = sorted(
    ...
    if not chunk_files:
        raise FileNotFoundError("No chunk files found in the specified directory.")
    
    with open(output_file, 'wb') as outfile:
        for chunk_file in chunk_files:
            chunk_path = os.path.join(input_dir, chunk_file)
            if os.path.getsize(chunk_path) == 0:
                raise ValueError(f"Chunk file {chunk_file} is zero bytes. Check if it's correctly created.")
...
combine_files(weights_folder, 'combined_inception_v3_weights.weights.h5')
model.load_weights('combined_inception_v3_weights.weights.h5')
